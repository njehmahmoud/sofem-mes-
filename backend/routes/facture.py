...
def get_facture(of_id: int, type: str = "interne", db=Depends(get_db)):
    ...
    ht=PRIX_UNIT*of["quantite"]; tva=ht*TVA_RATE; ttc=ht+tva
    ...
    c.drawString(cols_p[2]+2*mm,y_cur+2.5*mm,f"{PRIX_UNIT:.3f} TND")
...
def get_facture_groupee(data: MultiOFRequest, db=Depends(get_db)):
    ...
    total_ht  = sum(PRIX_UNIT * of["quantite"] for of in ofs)
    ...
    for idx,of in enumerate(ofs):
        ht=PRIX_UNIT*of["quantite"]; rh=8*mm; y_cur-=rh
        if idx%2==0: c.setFillColor(LIGHT); c.rect(15*mm,y_cur,W-30*mm,rh,fill=1,stroke=0)
        ...
        c.drawString(cols[3]+2*mm,y_cur+2.5*mm,f"{PRIX_UNIT:.3f}")
