"""SOFEM MES v6.0 — Analytics Routes"""
...
def analytics_production(db=Depends(get_db)):
    ...
    # Coût revient moyen par produit (top 5)
    try:
        couts = q(db, """
            SELECT p.nom produit_nom,
                   COUNT(o.id) nb_ofs,
                   ROUND(AVG(
                     (SELECT COALESCE(SUM(ob.quantite_requise * m.prix_unitaire),0)
            ...
            GROUP BY p.id ORDER BY nb_ofs DESC LIMIT 5
        """)
    except: couts = []
...
def analytics_achats(db=Depends(get_db)):
    ...
    # Top fournisseurs par volume commandes
    fournisseurs = q(db, """
    ...
    try:
        ...
        par_mois = q(db, """
            SELECT DATE_FORMAT(date_controle,'%Y-%m') mois,
                   ...
                   SUM(quantite_controlée) total_controlees,
