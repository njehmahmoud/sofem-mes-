    ...
    $('da-tb').innerHTML = (das||[]).length===0 ? empty(10) : das.map(da => {
      ...
      return `<tr>
        ...
        <td style="display:flex;gap:3px;flex-wrap:wrap">
          ...
          ${da.statut==='PENDING'
            ? <button class="fbtn" style="color:var(--green)" onclick="updateDA(${da.id},'APPROVED')" title="Approuver">✓</button>        <button class="fbtn" style="color:var(--red)"   onclick="updateDA(${da.id},'REJECTED')" title="Rejeter">✗</button>
            : ''}
