// ── materials.js ─────────────────────────────────────────
let _fournisseursList = [];

async function loadMaterials() {
  try {
    const [mats, mvts, fourns] = await Promise.all([
      api('/api/materiaux'),
      api('/api/materiaux/mouvements?limit=20'),
      api('/api/fournisseurs')
    ]);
    _fournisseursList = fourns || [];

    // Materials table
    $('mats-tb').innerHTML = (mats||[]).length === 0 ? empty(8) : (mats||[]).map(m => {
      ...
      return `<tr>
        ...
        <td style="font-family:'IBM Plex Mono',monospace;font-size:10px;color:${prix>0?'var(--accent)':'var(--muted)'}">
        ...
        <td style="display:flex;gap:3px">
          <button class="fbtn"
            ...
            onclick="openEditMateriauBtn(this)" title="Modifier">✎</button>
          <button class="fbtn" style="color:var(--red)"
            onclick="deleteMateriau(${m.id},'${m.nom.replace(/'/g,"\\'")}')">🗑</button>
        </td>
      </tr>`;
    }).join('');
    ...
    // Populate selects
    const fourOpts = '<option value="">— Aucun —</option>' +
      _fournisseursList.map(f => `<option value="${f.nom}">${f.nom}</option>`).join('');
    if ($('mv-mat'))      $('mv-mat').innerHTML = (mats||[]).map(m => `<option value="${m.id}">${m.nom} (${m.stock_actuel} ${m.unite})</option>`).join('');
    if ($('mat-fourn-sel'))  $('mat-fourn-sel').innerHTML  = fourOpts;
    if ($('mat-edit-four-sel')) $('mat-edit-four-sel').innerHTML = fourOpts;

    // Auto-generate next MAT code preview
    const codes = (mats||[]).filter(m=>m.code.startsWith('MAT-')).sort((a,b)=>b.code.localeCompare(a.code));
    const nextN = codes.length ? parseInt(codes[0].code.split('-')[1]||0)+1 : 1;
    if ($('mat-code-preview')) $('mat-code-preview').textContent = `MAT-${String(nextN).padStart(3,'0')}`;
...
async function saveMateriau() {
  if (!$('mat-nom').value) { toast('Nom obligatoire', 'err'); return; }
  try {
    const res = await api('/api/materiaux', 'POST', {
      code:          '',   // auto-generated by backend
      nom:           $('mat-nom').value,
      unite:         $('mat-unite').value  || 'pcs',
      ...
      fournisseur:   $('mat-fourn-sel')?.value         || null
    });
    toast(`${res.code||'Matériau'} créé ✓`); closeModal('m-mat'); loadMaterials();
...
function openEditMateriauBtn(btn) {
  ...
  // Set fournisseur dropdown
  const sel = $('mat-edit-four-sel');
  if (sel) sel.value = btn.dataset.four || '';
...
async function saveEditMateriau() {
  ...
  try {
    await api(`/api/materiaux/${id}`, 'PUT', {
      ...
      fournisseur:   $('mat-edit-four-sel')?.value        || null
    });
